...
def loadImages():
    ...
    for piece in pieces:
        images[piece] = p.transform.scale(p.image.load("C:/Users/Acer/Desktop/Data Science/Semester 5/AI Chess Project/Chess/images/" + piece + ".png"), (sq_size, sq_size))
    #We can now access an image by saying 'images['wp']'
    
"""
The main driver for our code. This willhandle user input an updating the grphics
"""
def main():
    p.init()
    ...
    running = True
    sqSelected = () #no square is selected initially, and also will keep track of the last click of the user in a (tuple: (row, col))
    ...
    while running:
        ...
        for e in p.event.get():
            ...
            #key handlers
                ...
                if e.key == p.K_z: #undo when 'z' is present
                    gs.undoMove()
                    sqSelected = ()
                    playerClicks = []
                    moveMade = True
        ...
        if not gameOver and not humanTurn and not moveUndone:
            if not AIThinking:
                AIThinking = True
                print("thinking...")
                returnQueue = Queue() #used to pass dta between threads
...
"""
Draw the squares on the board. The top left squareis always light.
"""
...
def highlightSquares(screen, gs, validMoves, sqSelected):
    if sqSelected != ():
        ...
        if gs.board[r][c][0] == ('w' if gs.whiteToMove else 'b'): #sqSelected is a piece that can be moved
            ...
            s.set_alpha(100) #transparency value, if 0 transparent and 255 opaque
            s.fill(p.Color('blue'))
